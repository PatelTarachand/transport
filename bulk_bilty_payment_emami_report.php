                      ...
                      <?php 
                        ...
                        if($ownerid=='' && $voucher_No==''){
                           // echo "SELECT * from m_truck as A left join bidding_entry as B on (A.truckid = B.truckid)   $cond and B.is_complete=1 && B.consignorid=4 && B.recweight !='0' && B.deletestatus!=1 Group by B.voucher_id,order by B.voucher_id  DESC";
                             $sel = "SELECT * from m_truck as A left join bidding_entry as B on (A.truckid = B.truckid)   $cond and B.is_complete=1 &&  B.recweight !='0'  && B.compid='$compid' and B.deletestatus!=1 Group by B.voucher_id order by B.voucher_id  DESC ";
                        }
                        else
                        {
                             $sel = "SELECT * from m_truck as A left join bidding_entry as B on (A.truckid = B.truckid)   $cond and B.is_complete=1 && B.recweight !='0' && B.deletestatus!=1 && B.compid='$compid' Group by B.voucher_id order by B.voucher_id  DESC";
                        ...
                        $vou_query="SELECT `payment_vochar` FROM `bulk_payment_vochar` WHERE `bulk_voucher_id`='$voucher_id'";
                                ...
                                $voures = mysqli_query($connection,$vou_query);
                                    ...
                                    $voucharno= $vocrow['payment_vochar'];
                                       ...
                                       $voucher_amount= showBiltyVoucher($connection,$compid,$voucher_id);	
                                       $payamount = $cmn->getvalfield($connection,"bilty_payment_voucher","sum(payamount)","ownerid='$ownerid' && voucher_id='$voucher_id' ");
