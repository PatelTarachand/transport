...
function showBiltyVoucher($connection,$compid,$voucher_id,$sessionid){
	$voucher_amount=0;
	 $sel = "SELECT * from bidding_entry where voucher_id = '$voucher_id' && compid='$compid' && sessionid='$sessionid'";
...
function showBiltyVoucher1($connection,$compid,$voucher_id,$sessionid){
	$voucher_amount=0;
	 $sel = "SELECT * from returnbidding_entry  where voucher_id = '$voucher_id' && compid='$compid' && sessionid='$sessionid'";
                                            $res = mysqli_query($connection,$sel);
                                    while($row = mysqli_fetch_array($res))
                                    {  
                     $adv_cash = $row['adv_cash'];
...
// {
// 	//echo $authKey; die;
//     $getData = 'mobileNos='.$mobileNumber.'&message='.urlencode($message).'&senderId='.$senderId.'&routeId='.$routeId.'&smsContentType=Unicode';
...
//	echo "Type: " . $_FILES[$docname]["type"] . "<br />";
    ...
    || ($docname["type"] == "image/GIF"))
  {
		...
		$doc_name=$tm.".".$ext;
		//echo "../admin/uploaded/$doc_name";die;
...
function uploadDoc($imgpath,$docname)
{
	//echo $imgpath;die;
	//print_r($docname);
	//die;
    ...
    || ($docname["type"] == "image/GIF"))*/
     ...
     if(($docname["type"] == "application/msword")
      || ($docname["type"] == "application/rtf")
	  || ($docname["type"] == "application/vnd.ms-excel")
      || ($docname["type"] == "application/pdf")
	  || ($docname["type"] == "image/jpg")
	  || ($docname["type"] == "image/jpeg")
	  || ($docname["type"] == "image/gif")
      || ($docname["type"] == "image/JPG")
	  ...
	  || ($docname["type"] == "image/PNG")
      || ($docname["type"] == "image/GIF")
	  || $docname["type"] == "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
	 
	 {
		...
		if(copy($docname['tmp_name'],$imgpath."$doc_name"))//$imgpath."$doc_name"
		{
			return($doc_name);
		}
		else
