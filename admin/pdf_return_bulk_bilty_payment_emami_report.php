...
$c_logo = $cmn->getvalfield($connection,"m_company","c_logo","compid='$_SESSION[compid]'");
  ...
  $receive_date = dateformatindia($cmn->getvalfield($connection,"return_bilty_payment_voucher","receive_date","voucher_id='$bulk_voucher_id' && sessionid='$sessionid' && ownerid='$ownerid'"));
  ...
  $sql = mysqli_query($connection,"select * from m_truckowner where ownerid='$ownerid'");
	...
	// $headname = $cmn->getvalfield($connection,"other_income_expense","headname","head_id='$row[head_id]'");
     ...
     $voucher_amount= showBiltyVoucher1($connection,$compid,$bulk_voucher_id,$sessionid);	
     $payamount = $cmn->getvalfield($connection,"return_bilty_payment_voucher","sum(payamount)","ownerid='$ownerid' && voucher_id='$bulk_voucher_id' && sessionid='$sessionid' && compid='$compid'");
...
$pdf->SetY(10);
 ...
 $pdf->SetX(20);
                  ...
                  $OWNres = mysqli_query($connection,$OWN);
                               ...
                               $invoicquery = "SELECT * from  returnbidding_entry Where  voucher_id='$bulk_voucher_id' && compid='$compid' && sessionid='$sessionid' ";      
                                        
                  //  $invoicquery = "SELECT * from m_truck as A left join returnbidding_entry as B on (A.truckid = B.truckid)   $cond and B.is_complete=1 && B.recweight !='0' ";
 ...
 $paidto=$incrow['paidto']; 
      $remark=$incrow['remark']; 
                        $pdf->Cell(125,10,'Truck Owner: '.$ownername ,2,0,'L');
                                  ...
                                  //$sel = "SELECT * from m_truck as A left join returnbidding_entry as B on (A.truckid = B.truckid)   $cond and B.is_complete=1  && B.recweight !='0' ";
                        ...
                        $di_no=$row['di_no'];
                        if($pay_no!=''){
                        $pay_no = $cmn->getcode($connection,"returnbidding_entry","pay_no","1=1 and sessionid = $_SESSION[sessionid]  && consignorid=4");
                                            ...
                                            $pdf->SetFont('Arial','',8);
                                                ...
                                                $total_tds +=$tds_amt;
                                                    ...
                                                    $pdf->SetFont('Arial','B',8);
                                                     ...
                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(14,12,$total_tds_amount,1,0,'R');
                                                        $pdf->SetFont('Arial','B',8);
