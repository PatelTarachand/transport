...
$fromdate = $cmn->dateformatindia($cmn->getvalfield($connection,"returnbidding_entry as A left join returnbill_details as B on A.bid_id=B.bid_id","min(inv_date)","A.invoiceid='$billid'"));
$todate = $cmn->dateformatindia($cmn->getvalfield($connection,"returnbidding_entry as A left join returnbill_details as B on A.bid_id=B.bid_id","max(inv_date)","A.invoiceid='$billid'")); 


$sql = mysqli_query($connection,"select * from 	returnbill where billid = '$billid'");
if($sql)
{
    while($row = mysqli_fetch_assoc($sql))
    {
        ...
        $consigneeid = $row['consigneeid'];
         ...
         $consi_type = $row['consi_type'];
          $party_gst = $cmn->getvalfield($connection,"m_consignor","cong_gstin","consignorid='$consignorid'");
           $party_gst = $cmn->getvalfield($connection,"m_consignee","gstno","consigneeid='$consigneeid'");
           
        //find sortagr
        $sortagr = 0;
        $sql_bill = mysqli_query($connection,"select bid_id from returnbill_details where billid = '$billid' ");
        if($sql_bill)
        {
            while($row_bill = mysqli_fetch_assoc($sql_bill))
            {
                $sortagr += $cmn->getvalfield($connection,"returnbidding_entry","sum(sortagr)","bid_id = '$row_bill[bid_id]' ");
            }
        }
        //echo $sortagr;die;
        if($sortagr != '0')
    ...
    $stateid = $cmn->getvalfield($connection,"m_consignee","stateid","consigneeid = '$consigneeid'"); 
        }
...
//$pdf->SetX(13);
	...
	$pdf->Cell(280,4,"TEXT  INVOICE",'0',1,'C',0);
    }else {
	...
	$pdf->SetFont('Arial','',10);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(40,4,"Party GST NO.     :",'0',0,'L',0);
	$pdf->Cell(80,4,"$party_gst",'0',1,'L',0);
	
	
			$pdf->setX(200);
	$pdf->SetFont('Arial','',10);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(40,4,"State CODE         :",'0',0,'L',0);
	$pdf->Cell(80,4,"$state_code",'0',1,'L',0);
   ...
   $pdf->Cell(16,6,'U/L',1,0,'C',1);
	...
	$pdf->SetWidths(array(8,20,20,25,25,28,27,20,20,25,18,16,10,25));
...
$sn=1;
 ...
 $sel = "select *,DATE_FORMAT(tokendate,'%Y-%m-%d') as tokendate from  returnbill_details left join returnbidding_entry on returnbidding_entry.bid_id = returnbill_details.bid_id  where returnbidding_entry.invoiceid = '$billid' order by returnbidding_entry.di_no";
...
$tot_rate =0;
	...
	$pdf->Row(array($sn++,$lr_no,$cmn->dateformatindia($billtydate),$bilty_no,$truckno,$fromplace,$destination,$recweight,$rate,$total,$adv_consignor,number_format($ulamt,2),$gst,$net));
