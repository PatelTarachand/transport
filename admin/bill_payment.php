...
if(isset($_GET['consignorid']) && isset($_GET['billid']))
{
	 ...
	 $advamount = $cmn->getvalfield($connection,"bidding_entry","sum(adv_consignor)","consignorid='$_GET[consignorid]' && invoiceid='$_GET[billid]' ");
 	  $bill_amount= $cmn->get_total_billing_amt($connection,"$_GET[billid]");	  
	 $payamount = $cmn->getvalfield($connection,"consignor_payment","sum(payamount)","consignorid='$_GET[consignorid]' && billid='$_GET[billid]' ");
	 $totaltdsamt = $cmn->getvalfield($connection,"consignor_payment","sum(tdsamt)","consignorid='$_GET[consignorid]' && billid='$_GET[billid]' ");
	  $deduct_amount = $cmn->getvalfield($connection,"consignor_payment","sum(deduct_amount)","consignorid='$_GET[consignorid]' && billid='$_GET[billid]' ");
...
$payment_rec_no=$_POST['payment_rec_no'];
  ...
  $sql_update = "INSERT INTO consignor_payment set consignorid='$consignorid',billid='$billid',bill_amount='$bill_amount',payamount='$payamount',tdsper='$tdsper',tdsamt='$tdsamt',advamount='$advamount',deduct_amount='$deduct_amount',bal_amt='$bal_amt',receive_date='$receive_date',payment_rec_no='$payment_rec_no',narration='$narration',createdate='$createdate',sessionid='$sessionid',compid='$compid',userid='$userid',ipaddress='$ipaddress'"; 
			mysqli_query($connection,$sql_update);
			
			//$cmn->InsertLog($connection,$pagename, $modulename, $tblname, $tblpkey, $truckid,'insert');
			echo "<script>location='bill_payment.php?action=1&consignorid=$consignorid'</script>";
	}
}else{
$payment_rec_no = $cmn->getcode($connection,"consignor_payment","payment_rec_no","1=1 and sessionid = $_SESSION[sessionid] && compid='$compid'");
}
 ...
 <?php include("../include/top_menu.php"); ?> <!--- top Menu----->
	<div class="container-fluid nav-hidden" id="content">
    	...
    	<legend class="legend_blue"><i class="icon-edit"></i>&nbsp; Payment Receive by Consignor</legend>   
			...
			$sql_fdest = mysqli_query($connection,"select * from bill where consignorid='$_GET[consignorid]' and compid='$compid'and sessionid= '$sessionid'");
			while($row_fdest = mysqli_fetch_array($sql_fdest))
			{
				$advamount1 = $cmn->getvalfield($connection,"bidding_entry","sum(adv_consignor)","consignorid='$row_fdest[consignorid]' && invoiceid='$row_fdest[billid]' ");
 	
	 $tdsamt1 = $cmn->getvalfield($connection,"consignor_payment","sum(tdsamt)","consignorid='$row_fdest[consignorid]' && billid='$row_fdest[billid]' ");
		
				 $bill_amount1= $cmn->get_total_billing_amt($connection,"$row_fdest[billid]");
	 $payamount1 = $cmn->getvalfield($connection,"consignor_payment","sum(payamount)","consignorid='$row_fdest[consignorid]' && billid='$row_fdest[billid]' ");
	 $deduct_amount1 = $cmn->getvalfield($connection,"consignor_payment","sum(deduct_amount)","consignorid='$row_fdest[consignorid]' && billid='$row_fdest[billid]' ");
						...
						<div class="box box-bordered">
							...
							<div class="box-content nopadding">
								<table class="table table-hover table-nomargin dataTable dataTable-tools table-bordered">
                                    ...
                                    <?php
									$slno=1;
								//	echo "SELECT * FROM `consignor_payment` WHERE compid='$compid' && consignorid='$_GET[consignorid]' order by payid desc";
									$sel = "SELECT * FROM `consignor_payment` WHERE compid='$compid' && consignorid='$_GET[consignorid]' and sessionid='$sessionid' order by payid desc";
...
function getUrl(billid){
	window.location.href='?billid='+billid+'&consignorid='+<?php echo $_GET['consignorid'];?>
}
