...
if(isset($_GET['purchaseid']))
{
	...
	$supplier_id = $row['supplier_id'];
	 ...
	 $supplier_name = trim($cmn->getvalfield($connection,"supplier_master","supplier_name","supplier_id='$supplier_id'"));
...
class PDF_MC_Table extends FPDF
{
  ...
  var $aligns;
	function Header()
	{
		global $title1,$title2,$supplier_name,$mobile,$billno,$purchase_date,$gsttinno,$cname,$caddress,$phoneno,$purchase_type,$billtype;
		 // courier 25
		...
		$this->SetFont('courier','b',9);
	    $this->Cell(33,5,"BILL NO. : ",0,0,'R');
		$this->SetFont('courier','',9);
	    $this->Cell(20,5,$billno,0,1,'L');
		...
		$this->SetFont('courier','b',9);
	    $this->Cell(32,5,"DATE  ",0,0,'R');
		$this->SetFont('courier','',9);
	   $this->Cell(20,5,": ".$purchase_date,0,1,'L');
	   
		$this->SetFont('courier','b',9);
	    $this->Cell(26,5,"Purchase Type  ",0,0,'R');
		$this->SetFont('courier','',9);
	   $this->Cell(20,5,": ".$purchase_type,0,0,'L');
	    $this->Ln(10);	 
	}
	  // Page footer
	function Footer()
...
$pdf->Cell(61,6,'Item Name',1,0,'C',1);
$pdf->Cell(26,6,'Qty',1,0,'C',1);
$pdf->Cell(23,6,'Rate',1,0,'C',1);
$pdf->Cell(20,6,'Total Amt',1,1,'C',1);



$pdf->SetX(5);
$pdf->SetWidths(array(8,61,26,23,20,50));
			...
			while($row_get = mysqli_fetch_assoc($sql_get))
{
    ...
    $itemcategoryname = $cmn->getvalfield($connection, "item_master", "itemcategoryname", "item_id='$row_get[item_id]'");
	...
	$pdf->SetFont('Arial','',8);
	$pdf->SetTextColor(0,0,0);
	$pdf->Row(array($slno,strtoupper($itemcategoryname),$row_get['qty'],$row_get['rate'],$row_get['total_amt']));
...
$pdf->SetTextColor(0,0,0);
$pdf->Cell(95,5,'SUBTOTAL :',1,0,'R',0);
$pdf->Cell(43,5,number_format($total,2),'1',0,'R',0);
