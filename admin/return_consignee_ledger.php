...
$pagename = 'return_consignee_ledger.php';
...
<html>
    ...
    <td colspan="4" style="text-align:right;font-weight:bold;">Contact No: +91-<?php echo $companymob1;?></td>
			...
			$sql_fdest = mysqli_query($connection,"select * from returnbill where consigneeid='$_GET[consigneeid]'");
			while($row_fdest = mysqli_fetch_array($sql_fdest))
			{
			...
			<script>document.getElementById('billid').value = '<?php echo $_GET['billid']; ?>';</script>
					...
					<div>
                          ...
                          <div align="center">
                          <hr>
                          <strong>Consignee Ledger Details</strong>
                          
                         <div align="right" class="btn-btn-primary">
                             <?php
							if($consigneeid =='all')
						{?>
                              <a href="pdf_return_consignee_ledgerall.php?consigneeid=<?php echo $consigneeid;?>&fromdate=<?php echo $fromdate;?>&todate=<?php echo $todate;?>" target="_blank"> <button type="button" class="btn btn-primary"> Print PDF</button></a>
                                <?php } else {?> 
                               <a href="pdf_return_consignee_ledger.php?consigneeid=<?php echo $consigneeid;?>&fromdate=<?php echo $fromdate;?>&todate=<?php echo $todate;?>" target="_blank"> <button type="button" class="btn btn-primary"> Print PDF</button></a>     </div>                    <hr>
                          <?php } ?>  
                          <table width="98%">
                          	<tr>
                            	<td colspan="2">
 	...
 	// $bill_amount= $cmn->get_total_billing_amt($connection,"$row[billid]");	  
 	 	...
 	 	$availbill =  $cmn->getvalfield($connection,"return_consignee_payment","count(billid)","billid='$row[billid]' and compid='$compid'");
	 ...
	 $tdsamt1 = $cmn->getvalfield($connection,"return_consignee_payment","sum(tdsamt)","consigneeid='$row[consigneeid]' && billid='$row[billid]' ");
...
//	if($availbill==0 ){
                ...
                if($consigneeid == 'all'){
                  $sel1 = "SELECT * FROM `return_consignee_payment` WHERE sessionid='$sessionid' and compid='$compid' order by payid desc";

                }else{

                 
									$sel1 = "SELECT * FROM `return_consignee_payment` WHERE sessionid='$sessionid' and compid='$compid' $cond1 order by payid desc";
                }
