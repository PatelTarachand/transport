...
//	print_r($_FILES);die;
	...
	$c=1;
		...
		//	print_r($data);die;
			    $schoolname=$data[0]; 
			 	$X=$data[1];
            $count = $cmn->getvalfield($connection,"m_truckowner","ownerid","`ownername` ='$X'");
            
            if($count!=''){
			$form_data = mysqli_query($connection,"Insert `m_truck` SET  truckno='$schoolname', ownerid='$count' ");
            }			  
			  $action=1;
			  $process = "insert";
