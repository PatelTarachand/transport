<?php
  ...
  $tblname = "";
  $tblpkey = "";
  $pagename = "return_bulk_bilty_payment_emami_report2.php";
                      ...
                      <?php 
                        ...
                        if($ownerid=='' && $voucher_No==''){
                          //  echo "SELECT * from m_truck as A left join returnbidding_entry as B on (A.truckid = B.truckid)   $cond and B.is_complete=1 && B.consignorid=4 && B.recweight !='0' && B.deletestatus!=1 Group by B.voucher_id,order by B.voucher_id  DESC";
                              $sel = "SELECT * from m_truck as A left join returnbidding_entry as B on (A.truckid = B.truckid)   $cond and B.is_complete=1 && B.sessionid='$sessionid' &&  B.recweight !='0'  && B.compid='$compid' and B.deletestatus!=1 Group by B.voucher_id order by B.voucher_id  DESC";
                        }
                        else
                        {
                             $sel = "SELECT * from m_truck as A left join returnbidding_entry as B on (A.truckid = B.truckid)   $cond and B.is_complete=1 && B.sessionid='$sessionid' && B.recweight !='0' && B.deletestatus!=1 && B.compid='$compid' Group by B.voucher_id order by B.voucher_id  DESC ";
                        ...
                        $vou_query="SELECT `payment_vochar` FROM `return_bulk_payment_vochar` WHERE `bulk_voucher_id`='$voucher_id'  && compid='$compid' && sessionid= $sessionid";
                                ...
                                $voures = mysqli_query($connection,$vou_query);
                                    ...
                                    $voucharno= $vocrow['payment_vochar'];
                                       ...
                                       $payamount = $cmn->getvalfield($connection,"return_bilty_payment_voucher","sum(payamount)","ownerid='$ownerid' && compid='$compid' && sessionid= $sessionid  && voucher_id='$voucher_id' ");
	...
	{  
		...
		if(confirm("Are you sure! You want to delete this record."))
		{
		  //  alert(id);
			$.ajax({
			  type: 'POST',
			  url: 'return_bulk_bilty_payment_emami_report2_detele.php',
			  data: 'id='+id,
			  dataType: 'html',
