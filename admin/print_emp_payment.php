...
{
	$emppayment_id = $_GET['emppayment_id'];	
	$sql = "select * from emp_payment where emppayment_id ='$emppayment_id'";
    ...
    $empname = $cmn->getvalfield($connection,"m_employee","empname","empid='$empid'"); 
...
/*  
	...
	//echo $stateid ;
	 ...
	 $tot_product_count = $cmn ->getvalfield($connection,"emp_paymentdetail","count(*)","emppayment_id='$emppayment_id'");
...
class PDF_MC_Table extends FPDF
{
  ...
  var $aligns;
	function Header()
	{
		global $title1,$title2,$truckno,$pay_date,$empname,$issudate,$gsttinno,$cname,$caddress,$phoneno,$purchase_type,$meterread;
		 // courier 25
	    ...
	    $this->Cell(20,5,"Payment Date",0,0,'L');
		$this->SetFont('arial','b',9);
	    $this->Cell(60,5,"     : ".$pay_date,0,0,'L');
...
$pdf->Cell(50,6,'Employee Name',1,0,'C',1);
$pdf->Cell(28,6,'Expense Head',1,0,'C',1);
$pdf->Cell(20,6,'Pay Mode',1,0,'C',1);

$pdf->Cell(28,6,'Amonut',1,1,'C',1);
...
$pdf->SetFont('Arial','',6);
$slno = 1;
$sql_get = mysqli_query($connection,"Select * from emp_payment where emppayment_id='$emppayment_id'");
			while($row_get = mysqli_fetch_assoc($sql_get))
{
    ...
    $totalqty =$row_get['amount'];
									  											
	$pdf->SetX(5);	
	$pdf->SetFont('Arial','',8);
	$pdf->SetTextColor(0,0,0);
	$pdf->Row(array($slno,$empname,$exp_head,$row_get['payment_type'],$row_get['amount']));
...
$pdf->Cell(28,5,number_format($totalqty,2),1,0,'R',0);
