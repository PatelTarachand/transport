<?php
  ...
  $tblname = "";
  $tblpkey = "";
  $pagename = "bulk_bilty_payment_emami.php";
  $modulename = "Bulk Bilty Payment";
...
if(isset($_GET['bulk_voucher_id'])){
  ...
  if($from !='--' && $todate !='--') {
    ...
    if($sessionid==7){ 
    $payment_vouchar="SELECT * FROM `bulk_payment_vochar` where sessionid= '$sessionid' && compid= '$compid' group by bulk_voucher_id";
    ...
    if($sessionid==6){
    $payment_vouchar="SELECT * FROM `bulk_payment_vochar` where sessionid= '$sessionid' && compid= '$compid' ORDER by id DESC";
     $payres = mysqli_query($connection,$payment_vouchar);
                              $payrow = mysqli_fetch_array($payres);
...
$total1= array_sum($total_paid);
  ...
  $tiny_url = get_tiny_url('http://shivalilogistics.in/erp/admin/pdf_bulk_bilty_payment_emami_report1.php?ownerid='.$ownerid.'&voucharno='.$vocher_no.'&bulk_voucher_id='.$payNoIncre1.'&compid='.$compid.'&sessionid='.$sessionid);
                      ...
                      <tr>
                        ...
                        <td>
                          <select id="truckno" name="truckno" class="select2-me input-large" style="width:220px;">
                            <option value=""> - All - </option>
                            <?php 
                              $sql_fdest = mysqli_query($connection,"SELECT * from m_truck as A left join bidding_entry as B on (A.truckid = B.truckid)  where B.is_complete=0  && B.recweight !='0' && B.payment_date='0000-00-00'");
                              while($row_fdest = mysqli_fetch_array($sql_fdest))
                              {
                              ?>
              ...
              <div class="box">
                ...
                ?>
                  ...
                  <h4> <?php if($bulk_voucher_id!='') { ?> Updated <?php } else { ?> Payment  <?php } ?> Date:   
                   ...
                   $payment_date = $cmn->getvalfield($connection,"bidding_entry","payment_date","voucher_id='$bulk_voucher_id' && compid='$compid' && sessionid='$sessionid'");
                     $paidto = $cmn->getvalfield($connection,"bidding_entry","paidto","voucher_id='$bulk_voucher_id' && compid='$compid' && sessionid='$sessionid'");
                       $remark = $cmn->getvalfield($connection,"bidding_entry","remark","voucher_id='$bulk_voucher_id' && compid='$compid' && sessionid='$sessionid'");
                   }
                   ?>
              ...
              <div class="box box-bordered">
                <div class="box-title">
                  <h3><i class="icon-table"></i><?php echo $modulename; ?> Details</h3>
                   <a style="align-items: right; float: right;" href="pdf_bulk_bilty_payment_emami.php?ownerid=<?php echo $ownerid ?>&payinvo=<?php if($bulk_voucher_id!='') { echo $bulk_voucher_id; } else { echo $_GET['payinvo']; }
               ?>" target="_blank" >
                  ...
                  <table class="table"  width="100%">
                    ...
                    <tbody>
                     ...
                     if($ownerid!='' or $truckno!='' or $bulk_voucher_id!=''){
                      ...
                      if($bulk_voucher_id!=''){
                        $payment_vochar = $cmn->getvalfield($connection,"bulk_payment_vochar","payment_vochar","bulk_voucher_id='$bulk_voucher_id' && compid='$compid' && sessionid='$sessionid'");

                         $sel = "SELECT * from  bidding_entry Where  voucher_id='$bulk_voucher_id' && compid='$compid'  && sessionid='$sessionid'";

                      }else{
			
                         $sel = "SELECT * from m_truck as A left join bidding_entry as B on (A.truckid = B.truckid)   $cond and B.sessionid='$sessionid' and B.is_complete=0  && B.recweight !='0' && B.compid='$compid' && B.payment_date='0000-00-00'";
                        }                       
                        ...
                        $pay_no = $row['pay_no'];
                        if($pay_no!=''){
                        $pay_no = $cmn->getcode($connection,"bidding_entry","pay_no","1=1 and sessionid = $_SESSION[sessionid]");
