...
if(isset($_GET['issueid']))
{
	...
	$empid = $row['empid'];
	 ...
	 $driver = trim($cmn->getvalfield($connection,"issueentry","empid","issueid='$issueid'"));
  ...
  var $aligns;
	function Header()
	{
	   ...
	   $this->Cell(23,5,$issudate,0,1,'L');
		...
		$this->SetX(6);
		$this->SetFont('arial','b',9);
	    $this->Cell(25,5,"Meter Reading",0,0,'L');
		$this->SetFont('arial','b',9);
...
$pdf->Cell(9,6,'S.No.','1',0,'L',1);  
$pdf->Cell(50,6,'Item Name',1,0,'C',1);
$pdf->Cell(50,6,'Return Item ',1,0,'C',1);
$pdf->Cell(10,6,'Unit',1,0,'C',1);
$pdf->Cell(6,6,'Qty',1,0,'C',1);

$pdf->Cell(13,6,'Remark',1,1,'C',1);



$pdf->SetX(5);
$pdf->SetWidths(array(9,50,50,10,6,13));
$pdf->SetAligns(array("L","L","L","L","R"));


$pdf->SetFont('Arial','',6);
$slno = 1;
// echo "select * from issueentrydetail where issueid='$issueid'";die;
$sqlget = mysqli_query($connection, "select * from issueentrydetail where issueid='$issueid'");
			while($row_get = mysqli_fetch_assoc($sqlget))
{
		$amount=0;
	
	 
	   $issuedetailid = $row_get['issuedetailid'];
       $returnitem_id = $row_get['returnitem_id'];
       $purdetail_id = $row_get['purdetail_id'];
      
      $issueid = $row_get['issueid'];
      $unitname = $row_get['unitname'];
       $is_rep = $row_get['is_rep'];
      $excrec = $row_get['excrec'];
      $qty = $row_get['qty'];
      $remark1 = $row_get['remark1'];
      $stock = $row_get['stock'];
 ...
 $itemcategoryname = $cmn->getvalfield($connection, "items", "item_name", "itemid='$itemid'");
         $returnitem = $cmn->getvalfield($connection, "items", "item_name", "itemid='$returnitem_id'");
	...
	$serial_no = $cmn->getvalfield($connection, "purchasentry_detail", "serial_no", "purdetail_id='$row_get[purdetail_id]'");
     ...
     $unit_name = $cmn->getvalfield($connection,"m_unit","unitname","unitname='$unitname'");
      		...
      		$itemid = $cmn->getvalfield($connection, "purchasentry_detail", "itemid", "purdetail_id='$returnitem_id'");
		$returnitem = $cmn->getvalfield($connection, "items", "item_name", "itemid='$itemid'");
		$serial_no2 = $cmn->getvalfield($connection, "purchasentry_detail", "serial_no", "purdetail_id='$returnitem_id'");
		
		 if($is_rep!=""){
		$trye1= $returnitem.'/'.$item_category_name.'/'.$serial_no2;
		 }
									  											
	$pdf->SetX(5);	
	$pdf->SetFont('Arial','',8);
	$pdf->SetTextColor(0,0,0);
	$pdf->Row(array($slno,$itemcategoryname.'/'.$item_category_name, $trye1, $row_get['unitname'],$row_get['qty'],$row_get['remark1']));
...
$pdf->SetTextColor(0,0,0);
$pdf->Cell(109,5,'SUBTOTAL :',1,0,'R',0);

$pdf->Cell(16,5,number_format($totalqty,2),'1',0,'R',0);
$pdf->Cell(13,5,' ',1,0,'R',0);
