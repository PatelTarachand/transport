...
{
	$saleid = $_GET['saleid'];	
	$sql = "select * from sale_entry where saleid = '$saleid'";
	$res = mysqli_query($connection,$sql);
	$row = mysqli_fetch_array($res);
	$bill_type    =  $row['bill_type'];

	$sale_date   = $cmn->dateformatindia($row['sale_date']);
    ...
    $customer_name = $cmn->getvalfield($connection, "m_customer", "customer_name", "customer_id='$row[customer_id]'");
     ...
     $mobile=$cmn->getvalfield($connection,"m_customer","mobile_no","customer_id='$row[customer_id]'");
...
/*  
	...
	//echo $stateid ;
	 ...
	 $tot_product_count = $cmn ->getvalfield($connection,"saleentry_detail","count(*)","saleid='$saleid'");
...
class PDF_MC_Table extends FPDF
{
  ...
  var $aligns;
	function Header()
	{
		global $title1,$title2,$customer_name,$mobile,$billno,$sale_date,$gsttinno,$bill_type,$cname,$caddress,$phoneno,$purchase_type,$billtype;
		 // courier 25
	    ...
	    $this->Cell(24,5,"Customer name ",0,0,'L');
		$this->SetFont('arial','b',9);
	    $this->Cell(60,5,"  : ".strtoupper($customer_name),0,0,'L');
		
		//echo $mobile;die;
        
		
		$this->SetFont('arial','b',9);
	    $this->Cell(32,5,"Contact No. : ",0,0,'R');
		$this->SetFont('arial','b',9);
	    $this->Cell(5,5,$mobile,0,1,'L');
		
		$this->SetX(6);
		$this->SetFont('arial','b',9);
	    $this->Cell(19,5,"DATE",0,0,'L');
		$this->SetFont('arial','b',9);
	    $this->Cell(60,5,"     : ".$sale_date,0,0,'L');
		
		$this->SetFont('arial','b',9);
	    $this->Cell(33,5,"Sale Type  ",0,0,'R');
		$this->SetFont('arial','b',9);
	   $this->Cell(20,5,": ".$bill_type,0,1,'L');
			...
			while($row_get = mysqli_fetch_assoc($sql_get))
{
	...
	$serial_no = $cmn->getvalfield($connection, "saleentry_detail", "serial_no", "purdetail_id='$row_get[purdetail_id]'");
	
		$itemid= $cmn->getvalfield($connection, "saleentry_detail", "itemid", "purdetail_id ='$row_get[purdetail_id]'");
