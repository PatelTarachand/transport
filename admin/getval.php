...
function getdieselopening($connection,$currdate)
{
	 ...
	 $diesel_open_bal_date = strtotime($this->getvalfield($connection,"account_setting","diesel_open_bal_date","1=1"));
	 $currdate_str = strtotime($currdate);
	
	
	if($currdate_str > $diesel_open_bal_date)
	{	
			
	
		$tot=0;
		$sql = mysqli_query($connection,"select A.qty from diesel_purchase_entry_detail as A left join diesel_purchase_entry as B on A.d_purchase_id=B.d_purchase_id where B.invoice_date > '$diesel_open_bal' && A.type='Diesel'");
		...
		$tot_fill =0;
		$sql2 = mysqli_query($connection,"select adv_dieselltr from bilty_entry  where adv_date > '$diesel_open_bal' && petrol_pump_type='self'");
		...
		$sql3 = mysqli_query($connection,"select from diesel_demand_detail as A left join diesel_demand_slip as B on A.dieslipid=B.dieslipid left join inv_m_supplier as C on C.supplier_id=B.supplier_id where C.type='self' && B.diedate >'$diesel_open_bal'");
		while($row3=mysqli_fetch_assoc($sql3))
		{
			$tot_fill_dd +=$row3['qty'];
		}
		
		$curr_openingbal = $openingbal + $tot - $tot_fill - $tot_fill_dd;
	}
	else
	{
...
function getcashopening($connection,$currdate)
{
	...
	else
	{
	 ...
	 $cashopeningbal = $this->getvalfield($connection,"account_setting","cashopeningbal","open_bal_date <='$currdate'");
		...
		$bilty_adv = $this->getvalfield($connection,"bilty_entry","sum(adv_cash)","adv_date between '$open_bal_date' and '$currdate' ");
		...
		$commision = $this->getvalfield($connection,"bilty_entry","sum(commission)","cashbook_date between '$open_bal_date' and '$currdate' ");
		$truck_owner_payment = $this->getvalfield($connection,"bilty_payment","sum(payamount)","paymentdate between '$open_bal_date' and '$currdate' ");
...
{    
	...
	$row_company = mysqli_fetch_array(mysqli_query($connection,"select compid from details_user where user_id='$user_id'"));
	$compid = $row_company['compid'];
	...
	$sql_priv = "select status from process_set_privilege where  compid='$compid' and page_id='$page_id' and role_id = '$role_id'" ;
