	 ...
	 $bilty_no = $_POST['bilty_no'];
	 	...
	 	$packingqty = $_POST['packingqty'];
				...
				//  $mobileNumber = $cmn->getvalfield($connection," m_consignee","phoneno","consigneeid='$consigneeid'");
				 $mobileNumber='7000796428';

				//  $msg="Dear $customer  Your Truck $truckno Loaded $itemname On $inv_date From $placename To  $toplacename , QTY $noofqty Party is $consigneename contact $mobileNumber. To Download Your Builty click $link SVLGST"; 
				 $msg="Dear";
    // $increasebiltyno=$cmn->sendsmsGET($connection,"bidding_entry","bilty_no","compid = '$_SESSION[compid]' && sessionid='$sessionid'");
       ...
       <legend class="legend_blue"><a href="dashboard.php"> 
    	...
    	<h4 style="color:#FF0000;"> <?php echo $duplicate; ?></h4>
		   ...
		   <div class="innerdiv">
			...
			<div class="text">
			    	<input type="text" class="formcent" name="lr_no" id="lr_no"  value="<?php	echo $lr_no;  ?>" required style="border: 1px solid #368ee0" autocomplete="off"  >
			...
			<div> Bilty No </div>
			<div class="text">
			<input type="text" class="formcent" name="bilty_no" id="bilty_no" value="<?php  if($bilty_no!='')
			                                                                        {  echo $bilty_no; }
		...
		<br>
			...
			<input type="text" class="formarea control_dynamic" name="drivermobile" id="drivermobile" value="<?php echo $drivermobile;?>" style="border: 1px solid #368ee0" autocomplete="off" maxlength="10" >                   
				...
				<div class="innerdiv">
                    ...
                    <div class="text">
                     <input type="text" class="formcent" name="ewayno" id="ewayno" value="<?php echo $ewayno; ?>"  style="border: 1px solid #368ee0"  autocomplete="off"  >                
                </div>
                </div>
                ...
                <div class="row-fluid">
					<div class="span12">
						...
						<div class="box-title">
							...
							//	echo "select *,DATE_FORMAT(tokendate,'%d-%m-%Y') as tokendate  from bidding_entry $cond && sessionid='$sessionid' && is_bilty=1 && consignorid='$consignorid' and compid='$compid' order by bid_id desc limit 500";
						...
						<!-- <input type="button" class="btn btn-primary" name="add_data_list" id="add_data_list" onClick="edit('<?php echo $row['saleid']; ?>');" value="E">
						  <input type="button" class="btn btn-primary" style="display: none;" name="add_data_list" id="add_data_list_edit_<?php echo  $row['saleid'] ; ?>" onClick="cted('<?php echo $row['saleid']; ?>','<?php echo $row['rate']; ?>','<?php echo $row['unitrate']; ?>','<?php echo $row['qty']; ?>','<?php echo $row['weight']; ?>','<?php echo $row['suppartyid']; ?>','<?php echo $row['unitid']; ?>','<?php echo $row['productid']; ?>','<?php echo $cmn->dateformatindia($row['billdate']); ?>');" value="E">
						  &nbsp; -->
