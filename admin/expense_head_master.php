...
$tblname = "exp_head_master";
$tblpkey = "exheadid";
$pagename = "expense_head_master.php";
$modulename = "Expense Master";
...
{
	$exheadid=$_GET['edit'];
	$sql_edit="select * from exp_head_master where exheadid='$exheadid'";
...
else
{
	...
	$narration='';
		...
		{
			$tblpkey_value= trim(addslashes($_POST['exheadid']));
			
			$exp_head = trim(addslashes($_POST['exp_head']));
			...
			if($exheadid =='')
			{
				...
				mysqli_query($connection,"insert into exp_head_master set exp_head='$exp_head',narration='$narration',sessionid='$sessionid'");
				$action = 1;
			}
			else
			{
				mysqli_query($connection,"update exp_head_master set exp_head='$exp_head',narration='$narration' where exheadid='$tblpkey_value'");
				$action = 2;
			}			
	...
	<div class="container-fluid" id="content">		
		...
		<img src="menu/home.png" style="width:30px;"/><span style="color:#FF0000; font-weight:bold"> GO TO HOME </span></a></legend> -->
                            ...
                            <select id="head_id" name="head_id" class="formcent select2-me" style="width:224px;" >
                                <option value=""> -Select- </option>
                                <?php 
                                $sql_fdest = mysqli_query($connection,"select head_id,exp_head from other_income_expense where headtype='Truck' && exp_head not IN('RTO & INSURANCE','Spare Item','total tax','Hamali','Bhatta','Cash Expanses','Truck Maintanance')  order by exp_head");
                                while($row_fdest = mysqli_fetch_array($sql_fdest))
                                {
                                ?>
                                    <option value="<?php echo $row_fdest['head_id']; ?>"><?php echo $row_fdest['exp_head']; ?></option>
                                <?php
                                } ?>
                                </select>
                                <script>document.getElementById('head_id').value = '<?php echo $head_id; ?>';</script>
                                 ...
                                 <tr>
                                    ...
                                    <td>
                    				<input type="text" name="exp_head" value="<?php echo $exp_head; ?>" id="exp_head" autocomplete="off"  class="input-large">
                                    </td>
                                   </tr>
                    ...
                    <input type="hidden" name="exheadid" id="exheadid" value="<?php echo $exheadid; ?>" >
                    			</td>
                                </tr>  
						...
						<div class="box box-bordered">
							...
							<div class="box-content nopadding">
								<table class="table table-hover table-nomargin dataTable dataTable-tools table-bordered">
									...
									$sql = "select * from exp_head_master order by exheadid desc";
									...
									?>
										<tr>
                                            <td><?php echo $slno; ?></td>
                                            <td><?php echo ucfirst($row['exp_head']);?></td>                                           
                                           
                                             <td><?php echo ucfirst($row['narration']);?></td> 
                                         
                                            <td class='hidden-480'>
											
                                           <a href= "?edit=<?php echo ucfirst($row['exheadid']);?>"><img src="../img/b_edit.png" title="Edit"></a>
                                           &nbsp;&nbsp;&nbsp;
		...
		$.ajax({
		  ...
		  success: function(data){
			 ...
			 // alert('Data Deleted Successfully');
			  location=pagename+'?action=10&exheadid=<?php echo $exheadid; ?>';
			}
