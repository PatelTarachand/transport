...
$tblname = 'bill';
$tblpkey = 'billid';
$pagename  = 'billing_entry_emami.php';
$modulename = "Bill Generation";
//print_r($_SESSION);
$billno = $cmn->getcode($connection, "bill", "billno", "1=1 and sessionid = $_SESSION[sessionid] && compid='$compid'");
...
if (isset($_GET['edit'])) {
	...
	$sql_eidt = mysqli_query($connection, "Select * from bill where billid = '$billid'");
	if ($sql_eidt === FALSE) {
	...
	<div class="container-fluid nav-hidden" id="content">
		...
		<div class="container-fluid">
									...
									<?php if ($consi_type == 'Consignor') { ?>
										...
										<td id="d2" style="width: 30%;">
											<select name="consignorid1" id="consignorid1" class="select2-me input-medium" style="width:250px;">
												<option value=""></option>
												<?php
												$sql = mysqli_query($connection, "Select consignorid,consignorname from m_consignor");
									...
									<?php if ($consi_type == 'Consignee') { ?>
										<td id="d4" style="width: 30%;">
											<select name="consigneeid" id="consigneeid" class="select2-me input-medium" style="width:250px;">
												<option value=""></option>
												<?php
												$sql = mysqli_query($connection, "Select consigneeid,consigneename from m_consignee");
												while ($row = mysqli_fetch_assoc($sql)) {
												?>
			...
			?>
				<div class="row-fluid">
					<div class="span12">
						<form action="save_gstbilling_emami.php" method="post">
								...
								<div class="box-content nopadding" style="border-top: 2px solid #ddd;">
									<table align="center" class="table table-condensed table-bordered" style="width:100%">
										...
										<tr>
											...
											<?php if ($gstvalid == 'Yes') { ?>
												...
												<td><select name="gst_type" id="gst_type" onChange="gstType(this.value);" class="select2-me input-medium" style="width:250px;" required>
														<option value=""></option>
												...
												<td align="center"><input type="text" class="input-small" name="cgst_percent" style="color:#f00; font-weight:bold;text-align:center;" id="cgst_percent" value="<?php echo $cgst_percent; ?>" onChange="setTotal();"><strong>%</strong></td>

												<td align="center"><input type="text" class="input-small" name="sgst_percent" style="color:#f00; font-weight:bold;text-align:center;" id="sgst_percent" value="<?php echo $sgst_percent; ?>" onChange="setTotal();"><strong>%</strong></td>
											<?php } else { ?>
									...
									<table align="center" width="100%">
										<tr bgcolor="#CCCCCC" style="border:#333 solid 1px">
											...
											<td>
												<select name="bid_id" id="bid_id" class="select2-me input-medium" onChange="checkbidding_entry(this.value);">
													<option value=""></option>
													<?php
													if ($consi_type == 'Consignee') {
														$sql = mysqli_query($connection, "Select bid_id,lr_no,tokendate from bidding_entry where is_invoice='0' && consigneeid='$consigneeid' && compid='$compid' && sessionid =$sessionid  order by bid_id desc");
													} else {
													   // echo "Select bid_id,lr_no,tokendate from bidding_entry where is_invoice='0' && consignorid='$consignorid' && compid='$compid'  && sessionid =$sessionid   order by bid_id desc";
														$sql = mysqli_query($connection, "Select bid_id,lr_no,tokendate from bidding_entry where is_invoice='0' && consignorid='$consignorid' && compid='$compid'  && sessionid =$sessionid   order by bid_id desc");
													}
													while ($row = mysqli_fetch_assoc($sql)) {
													?>
											...
											<td width="55%">&nbsp;</td>
										<td>
                                <input type="button" name="removebidding_entry" id="removebidding_entry" class="btn btn-danger" value="Remove Selected" onClick="funremovebidding_entry();" >
											</td>
											<input type="hidden" id="removelist" value="">
											<input type="hidden" id="addlist" name="addlist" value="">
											</td>
											<td align="right" width="12%">
												<input type="text" name="searchbidding_entry" id="searchbidding_entry" class="input-medium" value="" placeholder="Search Billty No.">
											</td>
										</tr>
									</table>
									<table border="1" width="100%" align="center" id="bidding_entrylist">
										<thead>
										...
										<tbody>
											...
											if ($billid != 0) {
												...
												$sel = "select * from  bill_details left join bidding_entry on bidding_entry.bid_id = bill_details.bid_id  where bidding_entry.invoiceid = '$billid' && bidding_entry.compid='$compid' ";
...
<input type="button" class="btn btn-danger" name="add_data_list" style="display: none;" id="add_data_deleteS1_<?php echo  $row['billdetailid']; ?>"  onClick="funDel1(<?php echo $row['billdetailid']; ?>,<?php echo $row['bid_id']; ?>);" value="X"></td>
											...
											$bidding_entrylist = rtrim($bidding_entrylist, ",");
											?>

											<script>
												document.getElementById('addlist').value = '<?php echo $bidding_entrylist; ?>';
											</script>
									...
									<?php if ($ispaid == '0') { ?>
										<table style="text-align:center;">
											...
											<tr>
												...
												<td>
													<a href="billing_entry_emami.php" class="btn btn-danger" style="border-radius:4px !important;">Cancel</a>
												</td>
											</tr>
		...
		<div class="row-fluid" id="list">
			<div class="span12">
				...
				<div class="box box-bordered">
					...
					<div class="box-title">
						<h3><i class="icon-table"></i><?php echo $modulename; ?> Details&nbsp;<span class="red">( Only 100 records are shown below for more <a href="billing_report.php?consignorid=<?php echo $consignorid; ?>" target="_blank">Click Here</a>)</span></h3>
...
function funDel(billid) {
			...
			if (confirm("Are you sure! You want to delete this record.")) {
				$.ajax({
					type: 'POST',
					url: '../ajax/removedeleteinvoice.php',
