...
$openingbal = $cmn->getcashopeningplant($connection,$curdate);
 

$cementadv = $cmn->getvalfield($connection,"bidding_entry as A left join m_truck as B on A.truckid=B.truckid left join m_truckowner as C on B.ownerid=C.ownerid","sum(A.adv_cash)"," A.adv_date='$curdate' ");

$clinkeradv = $cmn->getvalfield($connection,"bidding_entry as A left join m_truck as B on A.truckid=B.truckid left join m_truckowner as C on B.ownerid=C.ownerid","sum(A.adv_cash)","A.itemid=1 && A.adv_date='$curdate'  and ");
...
$otherincome = $cmn->getvalfield($connection,"otherincome as A left join m_userlogin as B on A.createdby = B.userid","sum(incomeamount)","A.payment_type='cash' && A.paymentdate='$curdate' && B.branchid=1   && A.createdby in (select userid from m_userlogin where )");
...
$otherexpense =0;
$sql = mysqli_query($connection,"select A.* from other_expense as A left join m_userlogin as B on A.createdby = B.userid where payment_type='cash' && paymentdate='$curdate' && branchid=1 && A.createdby in (select userid from m_userlogin) group by head_id"); 
while($row=mysqli_fetch_assoc($sql))
{
$head_id = $row['head_id'];
$headname = $cmn->getvalfield($connection,"other_income_expense","headname","head_id='$head_id'");

$expamount = $cmn->getvalfield($connection,"other_expense as A left join m_userlogin as B on A.createdby = B.userid","sum(expamount)","payment_type='cash' && paymentdate='$curdate' && branchid=1 && head_id='$head_id'  && A.createdby in (select userid from m_userlogin )");
...
$sql = mysqli_query($connection,"select A.* from truck_expense as A left join m_userlogin as B on A.createdby = B.userid where payment_type='cash' && paymentdate='$curdate' && branchid=1  && A.createdby in (select userid from m_userlogin) group by head_id"); 
while($row=mysqli_fetch_assoc($sql))
{
$head_id = $row['head_id'];
$headname = $cmn->getvalfield($connection,"other_income_expense","headname","head_id='$head_id'");
$expamount = $cmn->getvalfield($connection,"truck_expense as A left join m_userlogin as B on A.createdby = B.userid","sum(uchantiamount)","payment_type='cash' && paymentdate='$curdate' && branchid=1 && head_id='$head_id'  && A.createdby in (select userid from m_userlogin)");
