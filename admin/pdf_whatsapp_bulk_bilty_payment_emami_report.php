...
if(isset($_REQUEST['action']))
  ...
  if(isset($_GET['di_no']))
  {
    $di_no = trim(addslashes($_REQUEST['di_no']));  
  }
  else
  $di_no = '';
...
$saleid = 0; 
echo $filename = 'whatsapp/payment/'.$saleid.'.pdf';
  ...
  //   }
    ...
    $cond .=" and A.ownerid='$ownerid'";
       ...
       $bulk_voucher_id=$_REQUEST['voucher_id'];

    if($bulk_voucher_id !='') {
    
    $cond .=" and voucher_id='$bulk_voucher_id'";
    }
    
    
    $vocharid=$_REQUEST['voucharno'];
...
$c_logo = $cmn->getvalfield($connection,"m_company","c_logo","compid='$_SESSION[compid]'");
  ...
  $sql = mysqli_query($connection,"select * from m_truckowner where ownerid='$ownerid'");
	...
	// $headname = $cmn->getvalfield($connection,"other_income_expense","headname","head_id='$row[head_id]'");
     ...
     $payamount = $cmn->getvalfield($connection,"bilty_payment_voucher","sum(payamount)","ownerid='$ownerid' && voucher_id='$bulk_voucher_id' && sessionid='$sessionid' && compid='$compid'");
                                       ...
                                       $pdf->Cell(5,10,'',2,0,'L');
                                        ...
                                        $pdf->Ln(10);
                                            ...
                                            $pdf->SetFont('Arial','B',8);
                                                ...
                                                $total_tds=0;
                                                         $sn=1;
                                                        // echo "SELECT * from  bidding_entry Where  voucher_id='$bulk_voucher_id'  && compid='$compid' && sessionid='$sessionid' ";die;
