...
if($bid_id !='')
{
	$sql_Q= mysqli_query($connection,"select * from returnbidding_entry where bid_id='$bid_id'");
	...
	$personname = ucfirst($cmn->getvalfield($connection,"m_userlogin","personname","userid='$createdby'"));
		$brand_name = $cmn->getvalfield($connection,"brand_master","brand_name","brand_id='$brand_id'");
			...
			$m_packing = $cmn->getvalfield($connection,"inv_packing_master","m_packing","pid='$pid'");
   ...
   $consigneegstno = $cmn->getvalfield($connection,"m_consignee","gstno","consigneeid='$consigneeid'"); 

	$consignorname = $cmn->getvalfield($connection,"m_consignor","consignorname","consignorid='$consignorid'");
    
	$consignorcong_gstin = $cmn->getvalfield($connection,"m_consignor","cong_gstin","consignorid='$consignorid'"); 
...
$total_freight = $row['freightamt'];
    ...
    $total_balance=$total_freight - $adv_diesel-$adv_consignor;
...
class PDF_MC_Table extends FPDF
{
  ...
  var $aligns;
	...
	function Header()
	{		
		global $title1,$di_no,$pan_card,$tokendate,$gr_no,$ewayno,$companyname,$companyaddrs,$companymob1,$companymob2,$company_gst,$lr_no,$phoneno,$logo,$email_id;
        ...
        $this->Rect(137,26,68,13,'D');
		  ...
		  //headofficeblock
		$this->Rect(5,39,66,25,'D');  //consignor
		$this->Rect(71,39,66,25,'D');  //consignee
   ...
   $this->SetFont('arial','b',16);
//   $this->Image('logo/'.$c_logo,6,10,27,13);
   
      $this->SetFont('arial','b',16);
   $this->SetFont('arial','b',16);
   $this->Image('logo/'.$logo,36,130,88,7);//Image(string file [, float x [, float y [, float w [, float h [, string type [, mixed link]]]]]])
...
//$this->Image('bpslogo.png',80,3,35,16);//Image(string file [, float x [, float y [, float w [, float h [, string type [, mixed link]]]]]])
       ...
       // $this->Cell(190,3,$companyname.".",0,1,'L');
        
        
	$this->MultiCell(75,6,"$companyname",0,'L');
	...
	$this->MultiCell(85,4,$companyaddrs,0,'L');
    //$pdf->Cell(190,0,"$companyaddrs Office No : $companymob1, $companymob1",0,1,'L');
    
    $this->SetX(0);
    $this->SetY(34);
    $this->SetFont('arial','b',7);
    $this->Cell(180,0,"Email Id : ",0,0,'L');
		$this->SetY(32);
		$this->SetX(22);
		$this->SetFont('arial','b',8);
	    $this->MultiCell(60,4,"$email_id",0,'L');
	...
	function Footer()
	{
	    global $title1,$di_no,$pan_card,$tokendate,$gr_no,$ewayno,$companyname,$email_id,$personname;
		$this->SetY(-25);
	$this->SetX(7);
	...
	$this->SetY(123);
    $this->SetX(139);
	$this->Cell(10,5,"FOR",0,1,'L',0);
	$this->SetX(139);
	$this->Cell(138,5,$companyname,0,1,'L',0);
	 $this->SetX(165);
	$this->Cell(20,5,$personname,0,0,'L',0);
...
$pdf->AddPage('L','A5');
	...
	$pdf->SetX(5);
		$pdf->SetFont('arial','',7);
	$pdf->MultiCell(60,3,"$consignorname \n$consignoraddress\n $consignorcontactno",0,'L');
	
    $pdf->Ln(11);	
	$pdf->SetY(59);
    $pdf->SetX(5);
	$pdf->SetFont('arial','b',8);
	$pdf->Cell(15,0,"GST No :",0,1,'L');

	$pdf->SetX(23);
		$pdf->SetFont('arial','',7);
        
	$pdf->MultiCell(30,0,"$consignorcong_gstin ",0,'L');


			$pdf->SetY(62);
		 $pdf->SetX(5);
		 	$pdf->SetFont('arial','b',8);
		$pdf->Cell(180,0,"From : ",0,0,'L');
		$pdf->SetY(60);
		$pdf->SetX(16);
		$pdf->SetFont('arial','b',7);
	    $pdf->MultiCell(60,4,"$placename",0,'L');
		$pdf->Ln(2);
		
  	$pdf->SetY(62);
		 $pdf->SetX(40);
		 	$pdf->SetFont('arial','b',8);
		$pdf->Cell(180,0,"To: ",0,0,'L');
		$pdf->SetY(60);
		$pdf->SetX(46);
		$pdf->SetFont('arial','b',8);
	    $pdf->MultiCell(60,4,"$destination",0,'L');
		$pdf->Ln(2);
	...
	$pdf->SetFont('Arial','',8);
	$pdf->SetX(75);
	$pdf->MultiCell(65,4,"$consigneename \n$consigneeaddress  ",0,'L');
	$pdf->SetY(42);
	$pdf->SetX(96);
	$pdf->SetFont('arial','b',8);
	$pdf->Cell(190,0,"GST No.:",0,1,'L');	
	
	$pdf->SetFont('Arial','',7);
	$pdf->SetX(108);
	$pdf->MultiCell(80,0,"$consigneegstno  ",0,'L');
	
// 		$pdf->SetY(62);
// 		 $pdf->SetX(43);
// 		 	$pdf->SetFont('arial','b',10);
// 		$pdf->Cell(190,0,"To : ",0,0,'L');
// 		$pdf->SetY(60);
// 		$pdf->SetX(50);
// 	$pdf->SetFont('arial','',8);
// 	    $pdf->MultiCell(60,4,"$destination",0,'L');
		...
		$pdf->SetY(62);
		 $pdf->SetX(71);
		 	$pdf->SetFont('arial','b',8);
		$pdf->Cell(190,0,"Delivery At : ",0,0,'L');
			$pdf->SetY(60);
		$pdf->SetX(89);
	$pdf->SetFont('arial','b',8);
	    $pdf->MultiCell(60,4,"$destination",0,'L');
 ...
 //   $pdf->Cell(66,5,"Excise Invoice No : $invoiceno",1,1,'L',0);
	...
	$pdf->SetFont('arial','',11);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(68,5,"Driver Mo. No.  : $drivermobile",1,1,'L',0);
	...
	$pdf->Cell(24,6,'Item',1,0,'L',0);
			$pdf->Cell(15,6,'Brand',1,0,'L',0);
		$pdf->Cell(24,6,'Meth. of Packing',1,0,'L',0);

	$pdf->Cell(19,6,'Weight(bags)','1',0,'C',0); 
	$pdf->Cell(17,6,'Weight(MT)',1,0,'L',0);
	$pdf->Cell(15,6,'OWN Rate',1,0,'C',0);
	$pdf->Cell(18,6,'Freight Amt',1,1,'C',0);
	$pdf->SetFont('Arial','',6);
	
	$pdf->SetX(5);
	$pdf->SetFont('arial','b',6);	
	$pdf->Cell(24,8,$itemname,1,0,'L',0);
	$pdf->Cell(15,8,$brand_name,1,0,'L',0);
	$pdf->Cell(24,8,$m_packing."  $packingqty",1,0,'L',0);
	$pdf->Cell(19,8,$noofqty,1,0,'C',0);
	$pdf->Cell(17,8,$totalweight.' MT',1,0,'C',0);
	$pdf->Cell(15,8,$own_rate." / MT",1,0,'R',0);
	$pdf->Cell(18,8,number_format($total_freight,2),1,1,'R',0);
    ...
    $pdf->SetX(138);
    $pdf->Cell(50,5,"Mob.:  $ownermobileno ",0,1,'L');
    $pdf->SetX(138);
...
// 	$pdf->SetTextColor(0,0,0);
// 	$pdf->Cell(22,6,$packingqty,1,0,'L',0);
	...
	$pdf->Cell(38,6," Advance (Self) ",1,0,'L',0);
	$pdf->Cell(15,6,'',1,1,'R',0);
	$pdf->SetY(78);
	$pdf->SetX(112);
	$pdf->Cell(25,6,'',1,1,'R',0);
	
	$pdf->SetX(59);
	$pdf->SetFont('arial','b',9);
	$pdf->SetTextColor(0,0,0);
	 
	$pdf->Cell(38,6,"Advance(Consignor) ",1,0,'L',0);
	$pdf->Cell(15,6,'',1,1,'R',0);
	$pdf->SetY(84);
	$pdf->SetX(112);
	$pdf->Cell(25,6,number_format($adv_consignor,2),1,1,'R',0);
	...
	$pdf->SetY(90);
	$pdf->SetX(112);
	$pdf->Cell(25,6,number_format( $adv_diesel+$adv_consignor,2),1,1,'R',0);
	
	$pdf->SetY(96);
	$pdf->SetX(59);
	$pdf->Cell(38,6,"BALANCE TOTAL",1,0,'L',0);
	$pdf->Cell(15,6,'',1,1,'R',0);
	$pdf->SetY(96);
	$pdf->SetX(112);
	$pdf->Cell(25,6,number_format($total_balance,2),1,1,'R',0);
	
	$pdf->SetY(102);
	$pdf->SetX(5);
    $pdf->SetFont('arial','',7.5);   
  $pdf->MultiCell(132,4,'UNDERTAKING : Risks convered for T.P.N.D.L.R excluding breakage & leakage',1,'L'); 
