...
function getdayname($name) {
	...
	$loadamt = $this->getvalfield($connection,"customer_new_book","sum(debit)","supplier_id='$suppartyid' and cdate <='$fromdate'");
	...
	function getopeningbalcustNew($connection,$suppartyid,$fromdate) {
...
function getcarretopenbydatebetween($connection,$suppartyid,$unitid,$date,$todate) {
		
	$openbalcar = $this->getvalfield($connection,"openingunitentry","openunit","suppartyid='$suppartyid' and unitid='$unitid' && type=0");			
	$qtycartoon = $this->getvalfield($connection,"saleenetry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate between '$date' and '$todate'"); 
	$retaty = $this->getvalfield($connection,"carretentry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate between '$date' and '$todate' && is_sup=0");
	
	$purqtycartoon = $this->getvalfield($connection,"purchaseentrydetail as A left join purchaseentry as B on A.purchaseid=B.purchaseid","sum(A.qty)","B.suppartyid='$suppartyid' and A.unitid='$unitid' && purchasedate between '$date' and '$todate'");

	$purretaty = $this->getvalfield($connection,"carretentry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate between '$date' and '$todate' && is_sup=1");
	
	$loadingcaret = $this->getvalfield($connection,"loaderentry as A left join loading as B on A.lodingid=B.lodingid","sum(A.qty)","B.suppartyid='$suppartyid' and A.unitid='$unitid' && loaddate between '$date' and '$todate'");  
		...
		$sql = mysqli_query($connection,"select paymentid from payment where suppartyid='$suppartyid' and paymentdate='$row_get[date]' && type='sale'");
			...
			while($row=mysqli_fetch_assoc($sql)) {	
				...
				$payamount = $row['paid_amt'] + $row['discamt'];					
					...
					if($msg !='') {
					$nettoral=0;
					$newar = array($row_get['date'],$payamount,'','',$msg,$paymentid,$paid_amt,$discamt);					
					$roaker[] = $newar;
					}
			}
